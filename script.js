...
$("#test-respond").click(function () {
    ...
    if (sn > 0) {
        ...
        selectedRows.forEach(function (row, i) {
            var one = row.getData()
            setTimeout(function () {
                addr = "https://cors-anywhere.hgnet.workers.dev/?"+"//" + one.ip.replace(/\./g, "-") + urlprefix + pingUrl + "?" + Math.random()
...
$("#test-speed").click(function () {
    ...
    if (selectedRows.length > 0) {
        ...
        selectedRows.forEach(function (row) {
            ...
            sList.push({
                id: one.id,
                addr: "https://cors-anywhere.hgnet.workers.dev/?"+"//" + one.ip.replace(/\./g, "-") + urlprefix + imgUrls[imgi] + "?" + Math.random()
            })
        })
